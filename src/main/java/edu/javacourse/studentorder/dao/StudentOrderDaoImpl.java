...
public class StudentOrderDaoImpl implements StudentOrderDao {
    ...
    private static final String INSERT_ORDER =
            "INSERT INTO jc_student_order(" +
                    ...
                    "h_sur_name, h_given_name, h_patronymic, h_date_of_birth, h_passport_seria, h_passport_number, h_passport_date, h_passport_office_id, " +
                    "h_post_index, h_street_code, h_building, h_extension, h_apartment, " +
                    "h_university_id, h_student_number, " +
                    "w_sur_name, w_given_name, w_patronymic, w_date_of_birth, w_passport_seria, w_passport_number, w_passport_date, w_passport_office_id, " +
    ...
    public Long saveStudentOrder(StudentOrder so) throws DaoException {
        ...
        try (Connection con = getConnection();
            ...
            try {
                ...
                ResultSet genKeyRS = stmt.getGeneratedKeys();
                if (genKeyRS.next()) {
                    result = genKeyRS.getLong(1);
                }
                genKeyRS.close();
    ...
    private void setParamsForAddress(PreparedStatement stmt, int start, Person person) throws SQLException {
        Address person_adddress = person.getAddress();
        stmt.setString(start, person_adddress.getPostCode());
        stmt.setLong(start + 1, person_adddress.getStreet().getStreetCode());
        stmt.setString(start + 2, person_adddress.getBuilding());
        stmt.setString(start + 3, person_adddress.getExtension());
        stmt.setString(start + 4, person_adddress.getApartment());
    }
